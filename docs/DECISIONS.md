...
| G5 | D4 | `README.md:56` lists "An AST whitelist (`PythonDSLValidator`) for validating machine-authored blueprint DSL", with a bullet warning (as of this writing) that it "does not cover the `guard` step" because "a `condition:` is evaluated directly." That warning is now stale: D13 deleted `condition:`/`eval` from guard, so there is nothing left for the whitelist to fail to cover, and the bullet's own `#17` is the issue this PR closes. The residual gap: `PythonDSLValidator` (`validator_dsl.py`) still has no caller in the execute path — its only non-test reference outside its own module is the re-export at `src/bricks/core/__init__.py:58` — so crediting blueprint safety to it still conflates the `@flow` Python DSL path with the YAML path. ~~YAML guard conditions reach `eval` unchecked.~~ (struck 2026-09-07 — D13 deleted that `eval`; a guard is a brick call, so there is no guard string for the whitelist to cover or fail to cover.) |
